      ...
      {isLoading ? (
        <div className="space-y-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-5">
            {[1, 2, 3].map(i => (
              <div key={i} className="border border-slate-100 rounded-3xl p-6 bg-white shadow-[0_8px_30px_rgba(0,0,0,0.02)] flex flex-col justify-between min-h-[165px]">
                <div>
                  <div className="flex items-center justify-between">
                    {/* Icon on right */}
                    <div className="w-9 h-9 bg-slate-100 animate-pulse rounded-xl shrink-0" />
                    {/* Status / Tag on left */}
                    <div className="h-4 w-16 bg-slate-50 border border-slate-100 animate-pulse rounded-md" />
                  </div>
                  {/* Title and Code */}
            ...
            <div className="flex items-center justify-between w-full">
              <div className="flex items-center gap-2.5 min-w-0">
                <button 
                  ...
                  className="w-9 h-9 md:w-10 md:h-10 bg-white/80 hover:bg-white text-slate-600 hover:text-slate-900 rounded-[14px] md:rounded-2xl flex items-center justify-center border border-slate-100 hover:border-slate-200 cursor-pointer shadow-3xs transition-all duration-200 active:scale-95"
              ...
              <div className="flex items-center gap-2 shrink-0">
                ...
                <div className="relative shrink-0 group" data-recordings-dropdown>
                  <button
                    ...
                    className="w-9 h-9 md:w-10 md:h-10 bg-white/80 hover:bg-white hover:text-indigo-600 text-slate-600 border border-slate-100 hover:border-slate-200 rounded-[14px] md:rounded-2xl flex items-center justify-center cursor-pointer shadow-3xs transition-all duration-200 active:scale-95 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-indigo-500/20"
                    ...
                    {showRecordingsDropdown && (
                      <>
                        <motion.div
                          ...
                          className="fixed left-4 right-4 max-w-lg mx-auto top-[72px] md:absolute md:left-0 md:right-auto md:top-full md:mt-2 md:w-96 md:mx-0 md:max-w-none bg-white border border-slate-200 rounded-2xl shadow-[0_16px_48px_rgba(15,23,42,0.15)] z-50 overflow-hidden text-right font-sans"
                          ...
                          <div className="max-h-64 overflow-y-auto divide-y divide-slate-100 p-1.5">
                            {getClassRecordings(selectedClass.id).length === 0 ? (
                              <div className="text-center py-6 px-4 space-y-2">
                                <div className="w-8 h-8 bg-slate-50 border border-slate-100 rounded-xl flex items-center justify-center mx-auto text-slate-350">
                              ...
                              getClassRecordings(selectedClass.id).map((rec) => (
                                <div key={rec.id} className="p-2 hover:bg-slate-50 rounded-xl transition-colors flex flex-col gap-1.5">
                                  <div className="flex items-center justify-between gap-2">
                                    <div className="flex items-center gap-2">
                                      <button
                                        ...
                                        className="w-7 h-7 bg-indigo-50 hover:bg-indigo-100 border border-indigo-100 text-indigo-600 rounded-lg flex items-center justify-center shrink-0 transition-all cursor-pointer group"
                                        title="پخش صوت"
                                      >
                                  ...
                                  <div className="flex items-center justify-between gap-2 border-t border-slate-50/50 pt-1 mt-0.5">
                                    <span className="text-[8px] text-emerald-600 bg-emerald-50 border border-emerald-100 px-1 py-0.5 rounded font-bold">
                                      آماده تحلیل
                                    </span>
                                    <button
                                      onClick={() => {
                                      ...
                                      className="px-2 py-0.5 bg-slate-100 hover:bg-indigo-50 border border-slate-200/60 hover:border-indigo-100 text-slate-600 hover:text-indigo-600 text-[9px] font-black rounded-lg transition-all flex items-center gap-1 cursor-pointer"
                ...
                <div className="relative group">
                  <button
                    ...
                    className="w-9 h-9 md:w-10 md:h-10 bg-white/80 hover:bg-rose-50 hover:text-rose-600 text-slate-600 border border-slate-100 hover:border-rose-200 rounded-[14px] md:rounded-2xl flex items-center justify-center cursor-pointer shadow-3xs transition-all duration-200 active:scale-95 disabled:opacity-40 disabled:cursor-not-allowed disabled:hover:bg-white/80 disabled:hover:text-slate-400 disabled:border-slate-100 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-rose-500/20"
        ...
        <div className="space-y-6">
          ...
          {/* S_MAX WARNING */}
          {activeClasses.length >= 5 && (
            <div className="p-4 bg-amber-50 border border-amber-100 rounded-2xl flex items-start gap-3 text-amber-800 text-xs font-bold leading-relaxed shadow-sm">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-5">
            {classes.map((cls) => {
              ...
              return (
                <Card 
                  ...
                  className="border rounded-3xl p-6 bg-white transition-all flex flex-col justify-between min-h-[165px] group relative cursor-pointer border-slate-100 hover:border-indigo-100/80 shadow-[0_8px_30px_rgba(0,0,0,0.02)] hover:shadow-[0_12px_40px_rgba(0,0,0,0.04)]"
                >
                        ...
                        {activeDropdownId === cls.id && (
                          <>
                            <motion.div
                              ...
                              transition={{ duration: 0.15 }}
                              className="absolute left-0 mt-1 w-32 bg-white border border-slate-150/70 rounded-xl shadow-lg z-30 overflow-hidden text-right py-1 font-sans"
                              onClick={(e) => e.stopPropagation()}
                            >
                              <button
                                onClick={(e) => {
                  ...
                  <div className="flex items-center justify-between mt-6 pt-4 border-t border-slate-100 text-[10px] text-slate-450 font-black">
            ...
            {classes.length === 0 && (
              <div className="col-span-full text-center py-16 bg-white border border-slate-100 rounded-3xl shadow-[0_8px_30px_rgba(0,0,0,0.02)] space-y-4">
                <div className="w-14 h-14 bg-slate-50 border border-slate-150 rounded-2xl flex items-center justify-center mx-auto text-slate-350 animate-pulse">
            ...
            >
              <div className="flex items-center justify-between border-b border-slate-100 pb-3">
                ...
                <button 
                  ...
                  className="w-8 h-8 text-slate-450 hover:bg-slate-50 border border-slate-100 rounded-lg flex items-center justify-center cursor-pointer font-bold text-sm"
                >
                  ✕
                </button>
              </div>
              ...
              <form onSubmit={handleCreateClass} className="space-y-4">
                ...
                <div className="space-y-1.5">
                  ...
                  <div className="relative">
                    <select
                      ...
                      required
                      className="w-full bg-white border border-slate-150/60 rounded-xl px-3.5 py-2.5 text-xs text-slate-750 outline-none focus:border-indigo-500/80 focus:ring-4 focus:ring-indigo-500/5 font-bold cursor-pointer transition-all duration-200"
                    >
                ...
                <div className="space-y-1.5">
                  ...
                  <input
                    ...
                    className="w-full bg-white border border-slate-150/60 rounded-xl px-3.5 py-2.5 text-xs text-slate-750 outline-none focus:border-indigo-500/80 focus:ring-4 focus:ring-indigo-500/5 font-bold transition-all duration-200"
                  />
                </div>
                ...
                <div className="space-y-1.5">
                  ...
                  <input
                    ...
                    className="w-full bg-white border border-slate-150/60 rounded-xl px-3.5 py-2.5 text-xs text-slate-750 outline-none focus:border-indigo-500/80 focus:ring-4 focus:ring-indigo-500/5 font-mono text-left font-bold transition-all duration-200"
                  />
                </div>
                ...
                <div className="pt-4 border-t border-slate-100 flex items-center justify-end gap-2.5">
                  <button
                    type="button"
                    onClick={() => setShowCreateModal(false)}
                    className="px-4 py-2 border border-slate-100 hover:border-slate-200 text-slate-700 text-xs font-bold rounded-xl hover:bg-slate-50 transition-all cursor-pointer shadow-3xs"
                  >
                    انصراف
                  </button>
                  <Button
        ...
        {showDeleteConfirm && (
          <div className="fixed inset-0 bg-slate-900/50 backdrop-blur-xs flex items-center justify-center z-50 p-4 animate-in fade-in duration-250">
            <motion.div 
              ...
              animate={{ scale: 1, opacity: 1 }}
              exit={{ scale: 0.95, opacity: 0 }}
              className="bg-white border border-rose-100 rounded-3xl w-full max-w-md overflow-hidden shadow-[0_24px_60px_rgba(0,0,0,0.08)] p-6 space-y-5 text-right font-sans"
            >
              ...
              <div className="pt-4 border-t border-slate-100 flex items-center justify-end gap-2.5">
                <button
                  ...
                  className="px-4 py-2 border border-slate-100 text-slate-700 text-xs font-bold rounded-xl hover:bg-slate-50 cursor-pointer shadow-3xs"
                >
                  انصراف
                </button>
                <Button
        ...
        {transcriptRecording && (
          <div className="fixed inset-0 bg-slate-900/50 backdrop-blur-xs flex items-center justify-center z-50 p-4 animate-in fade-in duration-250">
            <motion.div 
              ...
              className="bg-white border border-slate-100 rounded-3xl w-full max-w-2xl overflow-hidden shadow-[0_24px_60px_rgba(0,0,0,0.08)] p-6 space-y-4 text-right font-sans relative"
            >
              <div className="flex items-center justify-between border-b border-slate-100 pb-3">
                <div className="flex items-center gap-2">
                ...
                <button 
                  ...
                  className="w-8 h-8 text-slate-450 hover:bg-slate-50 hover:text-slate-800 border border-slate-100 hover:border-slate-200 rounded-lg flex items-center justify-center text-sm cursor-pointer transition-colors"
                >
                  ✕
                </button>
              </div>
